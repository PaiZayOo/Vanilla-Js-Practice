...
const btnContainwr = document.querySelector('.btn-container');
...
function displayMenuBtns(){
  ...
  let menuBtn = categories.map(function(btn){
    return ` <button class="filter-btn" data-id=${btn}>${btn}</button>`;
  });
  menuBtn = menuBtn.join('');
  btnContainwr.innerHTML = menuBtn;
  const filterBtns = document.querySelectorAll('.filter-btn');
  filterBtns.forEach(function(btn){
    btn.addEventListener('click',function(e){
      const category = e.currentTarget.dataset.id;
      const menuCategorys = menu.filter(function(menuCategory){
